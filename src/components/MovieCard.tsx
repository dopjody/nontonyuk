                ...
                <div className="z-10 bg-[#181818] p-2 lg:p-4 absolute w-full shadow-lg rounded-b-md">
                    <div className="flex items-center gap-3">
                        <Link href={`/watch/${data.id}`} className="cursor-pointer w-6 h-6 lg:w-10 lg:h-10 bg-white rounded-full flex justify-center items-center hover:bg-neutral-300 transition">
                            <Play className="text-black w-3 h-3 lg:w-6 lg:h-6 fill-black ml-1" />
                        </Link>
                    ...
                    <Link href={`/watch/${data.id}`} className="block mt-4 gap-2">
                        <p className="text-green-400 font-semibold text-[10px] lg:text-sm inline mr-2">
                            98% Match
                        </p>
                        <p className="text-white text-[10px] lg:text-sm inline">{data.releaseDate?.split('-')[0] || '2025'}</p>
                    </Link>

                    <Link href={`/watch/${data.id}`} className="flex flex-row mt-4 gap-2 items-center hover:underline">
